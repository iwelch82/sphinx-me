...
def install():
    ...
    if type == '.rst':
        ...
        copy(join(getcwd(), "README.md"), join(docs_path, "index.md"))
    else:
        print()
        print("Unknown README file format found %s" % type)
    ...
    with open(join(docs_path, "conf.py"), "w") as f:
        ...
        f.write("html_theme_options = {'bootswatch_theme': 'yeti', }\n")
...
def setup_conf(conf_globals):
    ...
    print()
    print("\n".join([(k + ":").ljust(pad) + str(v) for k, v in settings.items()]))
    print()
